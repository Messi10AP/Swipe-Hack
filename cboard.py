...
while True:		
	__, im = cam.read()
	cv2.rectangle(im, (0,200), (1279,610), (255,0,0), 3)
	cv2.imshow("img", im)
	k=cv2.waitKey(33)
	if k == 27:
		break
...
im = im[200:610, 0:1279]
imgray = cv2.cvtColor(im,cv2.COLOR_BGR2GRAY)
temp =  cv2.erode(imgray,kernel,iterations = 1)
...
word=''
start = False
f = open("oldwords.txt", 'a')
g = open("newwords.txt", 'a')
while True:

	ret, img = cam2.read()
	img = img[200:610, 0:1279]
