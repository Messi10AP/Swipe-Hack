...
while True:
	try:
		...
		while True:
			#s = ser.readline()
			#if not int(s):
				#cv2.putText(final,"Keyboard removed.",(500,500), cv2.FONT_HERSHEY_SIMPLEX, 4,(255,0,0),2,cv2.LINE_AA)
				#cv2.imshow('Glyde', final)
				#badcount+=1
				#continue
		#	if badcount>10:
		#		cv2.waitKey(3000)
		...
		#		break
			...
			img = cv2.flip( img, 0 )
			img = cv2.flip( img, 1 )	
			img = img[220:720, 40:1240]
			roi = img[0:220, 0:1200]
			mask = np.ones(img.shape[:2], np.uint8)
			mask[360:500,0:1200] = 0
			...
			cnts = cv2.findContours(mask.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
			center = None
			cv2.putText(img, prev_word, (50, 420), cv2.FONT_ITALIC, 1,(255,255,255),1,cv2.LINE_AA)
			if len(cnts) > 0:
				...
				if radius > 50:
					...
					last_loc.append((int(x), int(y)))	
					cv2.circle(img, (int(x), int(y)), int(radius),(0, 255, 255), 2)
					cv2.circle(img, center, 5, (0, 0, 255), -1)
					#print (x,y)
					r = letter((x,y))
		 			cv2.putText(img,str(r),(int(x),int(y)), cv2.FONT_HERSHEY_SIMPLEX, 4,(255,0,0),2,cv2.LINE_AA)
				#cv2.waitKey(0)
					word+=r
				else:
					done+=1
			for i in range(len(last_loc)-1):
				cv2.line(img, last_loc[i], last_loc[i+1], (255,255,0),4)
			height, width, channels = img.shape
			final = cv2.resize(img, (0,0), fx=(float(1920)/(1.2*width)), fy=(float(1080)/(1.2*height)))
